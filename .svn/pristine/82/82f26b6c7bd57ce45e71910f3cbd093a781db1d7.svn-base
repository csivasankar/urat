 ...
 * @param attributes any additional attributes for the element (such as class
 *    or CSS styles or size
 *@Avinash - took out the function call and replaced it with "contains" from string built in
-->
<#macro formCheckboxesCustom path options separator attributes="">
    <@bind path/>
	    <#list options?keys as value>
		    <#assign id="${status.expression}${value_index}">
		    <#assign isSelected = (status.value?default([""]))?contains(value)>
		     <label class="checkbox" for="${id}">${options[value]?html}
...
<#-- 
 ...
 <#if Session['SPRING_SECURITY_CONTEXT']?exists>
   <#assign isAuthenticated=Session['SPRING_SECURITY_CONTEXT'].getAuthentication().isAuthenticated()/>
   ${isAuthenticated}
   <#else>
    false
  </#if>
